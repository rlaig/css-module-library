examples.lang = {
    ...
    resizeRight.className = 'c-resize--right';
      ...
      iframe.setAttribute('allow', 'clipboard-read; clipboard-write');
		...
		function addDialog() {
			var confirmDialog = '<div id="copy_dialog" class="fadeOut">Copied</div>';
			var dialogTemplate = document.createElement('div');
			dialogTemplate.innerHTML = confirmDialog;
			documentElement.querySelector('body').append(dialogTemplate);
		}

		function showDialog(title) {
			documentElement.querySelector('#copy_dialog').className = 'fadeIn';
			documentElement.querySelector('#copy_dialog').innerHTML = '"' + title + '" Copied!';
		}
		function hideDialog() {
			documentElement.querySelector('#copy_dialog').className = 'fadeOut';
		}
    ...
    iwin.addEventListener('load', function () {
		...
		const demoIcons = documentElement.querySelectorAll('div.demoIcon');
		demoIcons.forEach((obj) => {
			obj.onclick = function() {
				navigator.clipboard.writeText(obj.title);
				showDialog(obj.title);
